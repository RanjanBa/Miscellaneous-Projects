...
class Player(Character):
    def __init__(self, world: World, pos: Vector2, speed: float, radius: float, color=YELLOW):
        super(Player, self).__init__(world, pos, speed)
        self.radius = radius
        self.color = color
    ...
    def update(self, delta_time: float):
        ...
        if current_cell_pos.distance_to(self.position) < self.radius + self._world.food_radius:
            self._world.level_map[int(current_cell.x)][int(
                current_cell.y)].food = False
        ...
        self.target_cell = self._findTargetCell(current_cell,
                                                self.move_dir, self.desired_dir)
        ...
        dir = target_pos - self.position  # self._getMoveDirection()
